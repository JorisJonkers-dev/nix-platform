...
let
  ...
  wolfState = "/etc/wolf";
  # AMD variant: drop NVIDIA env + DeviceRequests, bind /dev/dri and
  # /dev/kfd, and use device-cgroup-rule c 226:* (DRM major) instead
  # of NVIDIA's c 244:*. Input device major (c 13:*) is unchanged.
  wolfSteamApp = ''
        [[profiles.apps]]
        title = "Steam"
        ...
        icon_png_path = "https://games-on-whales.github.io/wildlife/apps/steam/assets/icon.png"
            ...
            env = [
                ...
                "GOW_REQUIRED_DEVICES=/dev/input/* /dev/dri/* /dev/kfd"
            ]
            mounts = [
            ...
            base_create_json = """
            {
              "HostConfig": {
                ...
                "Privileged": false,
                "DeviceCgroupRules": ["c 13:* rmw", "c 226:* rmw"]
              }
            }
            """
  '';
  ...
  wolfBaseApps = ''
        ...
        icon_png_path = "https://games-on-whales.github.io/wildlife/apps/retroarch/assets/icon.png"
            ...
            env = [
                "RUN_SWAY=1",
                "GOW_REQUIRED_DEVICES=/dev/input/* /dev/dri/*"
            ]
            mounts = [
                "${gamesMount}:/ROMs:ro",
                "${gamesMount}:/games:ro",
  ...
  wolfConfigSeed = pkgs.writeText "wolf-config.toml" (
    ''
      hostname = "enschede-rx7900xtx-1"
      support_hevc = true
      config_version = 2
      uuid = "79000000-0000-4000-8000-0000000079e0"
...
in
{
  ...
  virtualisation.oci-containers = {
    ...
    containers.wolf = {
      ...
      environment = {
        ...
        WOLF_LOG_LEVEL = "INFO";
        # RDNA3 exposes a primary card (card0) and a render-only node
        # (renderD128). Wolf needs the render node; card0 is reserved
        # for the system / virtual TTY.
  ...
  systemd.tmpfiles.rules = [
    ...
    "d ${localGamesMount}/imports/rx7900xtx 0775 ${gameUser} users - -"
  ...
  assertions = [
    {
      ...
      message = "game-streaming-amd.nix requires the AMD GPU profile (hardware.graphics + amdgpu).";
    }
  ];
}
